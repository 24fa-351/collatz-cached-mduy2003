...
unsigned long long int get_from_lru_cache(LRUCache* cache,
    unsigned long long int key, int* cache_hits, int* cache_accesses)
{
    (*cache_accesses)++;
    for (int ix = 0; ix < cache->size; ix++) {
        if (cache->entries[ix].key
            == key) { // If key is found, move it to the front of the cache
            (*cache_hits)++;
...
void put_in_lru_cache(
    LRUCache* cache, unsigned long long int key, unsigned long long int value)
{
    if (cache->size < cache->capacity) { // If cache is not full, add the new
                                         // element to the front of the cache
        ...
        cache->size++;
    } else { // If cache is full, remove the last element and shift all element
             // to the right
...
unsigned long long int get_from_fifo_cache(FIFOCache* cache,
    unsigned long long int key, int* cache_hits, int* cache_accesses)
{
    (*cache_accesses)++;
    for (int ix = 0; ix < cache->size;
         ix++) { // Searches for key and returns value if found
...
void put_in_fifo_cache(
    FIFOCache* cache, unsigned long long int key, unsigned long long int value)
{
    cache->rear
        = (cache->rear + 1)
          % cache->capacity; // Add the new element to the rear of the cache
