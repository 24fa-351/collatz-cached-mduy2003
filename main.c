...
int main(int argc, char* argv[])
{
    ...
    for (size_t current_value = 0; current_value < num_of_values;
         ++current_value) {
        unsigned long long int random_number = rand() % (max - min + 1) + min;
        unsigned long long int num_of_steps
            = collatz_conjecture_wrapper(random_number, lruCache, fifoCache,
                cache_policy, &cache_hits, &cache_accesses);
