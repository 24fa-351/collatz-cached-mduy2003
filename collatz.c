...
unsigned long long int collatz_conjecture_wrapper(unsigned long long int value,
    LRUCache* lruCache, FIFOCache* fifoCache, const char* cache_policy,
    int* cache_hits, int* cache_accesses)
{
    unsigned long long int steps = 0;
    if (strcmp(cache_policy, "LRU") == 0) {
        ...
        steps
            = get_from_fifo_cache(fifoCache, value, cache_hits, cache_accesses);
        if (steps == 0) {
            steps = collatz_conjecture(value);
